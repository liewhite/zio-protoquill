...
trait Parser(rootParse: Parser | Parser.Nil)(using Quotes) {
  ...
  def apply(input: Expr[?])(using History): Ast = attempt.lift(input).getOrElse(error(input))
  protected def error(input: Expr[?]): Nothing = failParse(input, classOf[Ast])
  protected def attempt: History ?=> PartialFunction[Expr[?], Ast]
  ...
  private[engine] def attemptProper: History ?=> PartialFunction[Expr[?], Ast] = attempt
}
...
object Parser {
  ...
  def empty(rootParse: Parser | Parser.Nil)(using Quotes) =
    new Parser(rootParse) {
      protected def attempt: History ?=> PartialFunction[Expr[?], Ast] = PartialFunction.empty
    }
  ...
  trait PrefilterType[Criteria: Type](using Quotes) extends Parser {
    import quotes.reflect._
    def prefilter(expr: Expr[?]): Boolean =
  ...
  trait Prefilter(using Quotes) extends Parser {
    def prefilter(expr: Expr[?]): Boolean
    private[engine] override def attemptProper: History ?=> PartialFunction[Expr[?], Ast] =
      new PartialFunction[Expr[?], Ast] {
        def apply(expr: Expr[?]): Ast = attempt.apply(expr)
        def isDefinedAt(expr: Expr[?]): Boolean = prefilter(expr) && attempt.isDefinedAt(expr)
      }
  }
